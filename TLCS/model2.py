...
class DQNSetModel(tf.keras.Model):
    ...
    def call(self, lanes):
        ...
        b = tf.shape(lanes)[0]
        n = tf.shape(lanes)[1]
        flat = tf.reshape(lanes, [-1, tf.shape(lanes)[-1]])  # (batch * lanes, features)
        emb  = self.embedder(flat)                           # (batch * lanes, embedding_dim)
        emb  = tf.reshape(emb, [b, n, self.embedder.dense2.units])  # (batch, lanes, embedding_dim)
        return self.aggregator(emb)
                    # (batch, num_actions)
...
class TrainModelAggregator:
    ...
    def predict_one(self, state):
        """
        state: (num_lanes, lane_feature_dim) or (1, num_lanes, lane_feature_dim)
        returns: (1, num_actions)
        """
        if len(state.shape) == 2:
            state = np.expand_dims(state, 0)  # make it (1, 9, 9)
        elif len(state.shape) != 3:
            raise ValueError(f"Expected input shape (num_lanes, features) or (1, num_lanes, features), got {state.shape}")
    ...
    def load_model(self, path):
        # Build the model once by calling it with dummy input
        dummy_input = tf.random.uniform((1, 9, self._lane_feature_dim))  # (batch, num_lanes, features)
