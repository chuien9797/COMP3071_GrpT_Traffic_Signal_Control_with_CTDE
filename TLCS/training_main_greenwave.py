...
from training_simulation_greenwave import SimulationGreenWave
...
if __name__ == "__main__":
    ...
    sumo_cmd = set_sumo(config['gui'], 'sumo_config2.sumocfg', config['max_steps'])
    ...
    Simulation = SimulationGreenWave(
        Model,
        Memory,
        TrafficGen,
        sumo_cmd,
    ...
    while episode < config['total_episodes']:
        ...
        # For PPO, the epsilon value (used for epsilon-greedy in DQN) is not needed.
        if config.get('algorithm', 'DQN') == 'PPO':
            epsilon = 0.0  # or ignore, if your Simulation.run() method handles PPO differently
        else:
